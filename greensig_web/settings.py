...
if sys.platform == 'win32':
    # Essayer plusieurs chemins QGIS possibles
    QGIS_PATHS = [
        r'C:\Program Files\QGIS 3.40.13',
        r'C:\Program Files\QGIS 3.42.1',
    ...
    if OSGEO4W:
        ...
        # Trouver automatiquement la version de gdal*.dll
        import glob
        gdal_dlls = glob.glob(os.path.join(OSGEO4W, 'bin', 'gdal*.dll'))
        if gdal_dlls:
            # Prendre le premier gdal*.dll trouvé (ex: gdal310.dll)
            GDAL_LIBRARY_PATH = gdal_dlls[0]
        else:
            GDAL_LIBRARY_PATH = OSGEO4W + r'\bin\gdal310.dll'
        GEOS_LIBRARY_PATH = OSGEO4W + r'\bin\geos_c.dll'
    else:
        print("ATTENTION: QGIS/GDAL non trouvé. Installez QGIS depuis https://qgis.org/download/")
